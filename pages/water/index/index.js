...
//获取当前城市
  ...
  initBanner() { 
    var that = this;
    util.request('https://www.yunquanlai.com/client/api/getProductBanner', '', function (res) {
      var banner = (res.data.banner).split(',');
      that.setData({
        imgUrls: banner
      })
    });
...
//查询数据
      that.getGoodsData({
        url: 'https://www.yunquanlai.com/client/api/queryProduct?brandId=' + brandsID + '&bucketType=' + bucketID + '&waterType=' + waterID+'&orderType=' + sortID + '&isQuick=20&offset=0&limit=10',
        callback: function () { }
      });
  },
  //打开分类弹窗 
  ...
  //打开当前城市弹窗 
  showCurrentCityBox: function () {
    var state = util.modifyPopupBoxState(this.data.showCurrentCityBox)
    this.setData({
      showCurrentCityBox: state
    })
  },
  closeCurrentCityBox: function () {
    var state = util.modifyPopupBoxState(this.data.showCurrentCityBox)
    this.setData({
      showCurrentCityBox: state
    })
  ...
  onReachBottom: function () {
    ...
    util.request('https://www.yunquanlai.com/client/api/queryProduct?brandId=' + brandsID + '&bucketType=' + bucketID + '&orderType=' + sortID + '&isQuick=20&offset=' + offset + '&limit=10', '', function (res) {
      ...
      success: function (res) {
        ...
        qqmapsdk.reverseGeocoder({
          ...
          success: function (res) {
            ...
            if (city.indexOf('南宁') < 0) {
              that.showCurrentCityBox()
            }
            if (city.indexOf('市') > 0){
  ...
  gotoLocation(){
    wx.switchTab({
      url: '/pages/water/location/location',
    })
  }
})